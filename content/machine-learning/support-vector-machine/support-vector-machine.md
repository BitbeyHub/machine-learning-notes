...
核技巧的想法是，在学习与预测中只定义核函数K(x,z)，而不显式地定义映射函数Φ（**这样其实有一个问题就是，你得知道定义的这个函数K到底是不是核函数，如何验证？接下来会讲到**）通常，直接计算K(x,z)比较容易，而通过Φ(x)和Φ(z)计算K(x,z)并不容易。注意，**Φ是输入空间R^n到特征空间的映射，特征空间一般是高维的，甚至是无穷维的**。
