...
一般来说，假设进行kNN分类使用的样本特征是${(x_{i1},x_{i2},...,x_{in})}^m_{i=1}$，取每一轴上的最大值最小值
$$
...
在下一步中r=0+1=1mod2=1对应 y 轴，左右两边再按照 y 轴的排序进行切分，中位点记载于左右枝的节点。得到下面的树，左边的 x 是指这该层的节点都是沿 x 轴进行分割的。
...
导语和简介部分来自于此文章，适这篇文章适合小白入门kNN，但是这里就不放上来了，因为太简单了。
