...
好在还有更高效的算法。可以把$$B[i]=A[0]*A[1]*.....*A[i-1]*A[i+1]*...*A[n-1]$$。看成$$A[0]*A[1]*...*A[i-1]$$和$$A[i+1]*...*A[n-2]*A[n-11]$$两部分的乘积。

即通过$$A[i]$$项将$$B[i]$$分为两部分的乘积。效果如下图所示：

![array-66](pic/array-66.png)

不妨设定$$C[i]=A[0]*A[1]*...*A[i-1]$$，$$D[i]=A[i+1]*...*A[n-2]*A[n-21]$$。$$C[i]$$可以用自上而下的顺序计算出来，即$$C[i]=C[i-1]*A[i-1]$$。类似的，$$D[i]$$可以用自下而上的顺序计算出来，即$$D[i]=D[i+1]*A[i+1]$$。
